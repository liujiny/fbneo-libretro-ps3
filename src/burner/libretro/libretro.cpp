...
#ifdef __PS3__
extern "C" void fbneo_ps3_probe(const char*) {}
extern "C" void fbneo_ps3_probe_i(const char*, INT32) {}
#endif
...
#ifdef __PS3__
	HandleMessage(RETRO_LOG_WARN, "[FBNeo] PS3: dynamic DIP core options disabled; using game defaults.\n");
	return 0;
#endif
...
{
	// Keep the list loaded by create_variables_from_cheats(). Reloading here
	// detaches new nodes and leaks them whenever a core option changes.
...
static int archive_load_rom_chunk(uint8_t *dest, int *wrote, int i, UINT32 offset, UINT32 length)
{
	if (i < 0 || i >= nRomCount || pRomFind[i].nState != STAT_OK)
		return 1;
	...
	int ret = ZipLoadFileChunk(dest, (INT32)length, wrote, pRomFind[i].nPos, offset);
	BurnRomInfo ri = {0};
	BurnDrvGetRomInfo(&ri, i);
	if (ret != 0 || offset + length >= ri.nLen)
		ZipClose();
	return ret;
}
...
static bool open_archive()
{
	...
	if (g_find_list_path.size() > 0)
	{
		...
		BurnExtLoadRomChunk = archive_load_rom_chunk;
		return ret;
	}
	else
	{
...
{
#ifdef __PS3__
	// Legacy PSGL does not upload FBNeo big-endian XRGB8888 correctly.
	// RGB565 is native to this path and halves frame upload bandwidth.
	SetBurnHighCol(16);
#else
...
static bool retro_load_game_common()
{
	...
	if (nBurnDrvActive < nBurnDrvCount) {
		...
		if ((BurnDrvGetHardwareCode() & HARDWARE_PUBLIC_MASK) == HARDWARE_SNK_NEOGEO ||
			...
			HandleMessage(RETRO_LOG_INFO, "[FBNeo] PS3 64 MB memory pool initialized after driver selection (128-byte alignment).\n");
		}
#endif
...
#else
		HandleMessage(RETRO_LOG_INFO, "[FBNeo] PS3: INI cheat core options enabled; dynamic IPS/romdata options disabled.\n");
#endif
...
#endif
#ifndef FBNEO_DEFAULT_AUDIO_RATE
#define FBNEO_DEFAULT_AUDIO_RATE 48000
#endif
	nBurnSoundRate = FBNEO_DEFAULT_AUDIO_RATE;
	nBurnFPS = 6000;
	AudioBufferInit(nBurnSoundRate, nBurnFPS);
	RomDataExit();
	IpsPatchExit();
