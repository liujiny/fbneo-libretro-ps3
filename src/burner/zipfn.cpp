...
static INT32 nChunkEntry = -1;
static UINT32 nChunkOffset = 0;
static UINT32 nChunkLength = 0;
static bool bChunkFileOpen = false;
...
INT32 ZipClose()
{
	if (nFileType == ZIPFN_FILETYPE_ZIP) {
		if (bChunkFileOpen) {
			...
			bChunkFileOpen = false;
		}
		nChunkEntry = -1;
		nChunkOffset = nChunkLength = 0;
		if (Zip != NULL) {
			unzClose(Zip);
			Zip = NULL;
		}
...
// Read a range from a ZIP member without allocating a member-sized buffer.
// DEFLATE is sequential, so nOffset bytes are inflated into a small discard buffer.
// This saves peak RAM during driver initialization at the cost of extra I/O/CPU.
INT32 ZipLoadFileChunk(UINT8* Dest, INT32 nLen, INT32* pnWrote, INT32 nEntry, UINT32 nOffset)
{
	if (pnWrote) *pnWrote = 0;
	if (Dest == NULL || nLen < 0 || nFileType != ZIPFN_FILETYPE_ZIP || Zip == NULL) return 1;

	INT32 nRet;
	bool sequential = bChunkFileOpen && nChunkEntry == nEntry && nChunkOffset == nOffset;
	if (!sequential) {
		if (bChunkFileOpen) {
			unzCloseCurrentFile(Zip);
			bChunkFileOpen = false;
		...
		if (unzGetCurrentFileInfo(Zip, &info, NULL, 0, NULL, 0, NULL, 0) != UNZ_OK) return 1;
		if (unzOpenCurrentFile(Zip) != UNZ_OK) return 1;
		bChunkFileOpen = true;
		nChunkEntry = nEntry;
		nChunkOffset = 0;
		nChunkLength = info.uncompressed_size;

		UINT8 discard[16384];
		while (nChunkOffset < nOffset) {
			UINT32 want = nOffset - nChunkOffset;
			if (want > sizeof(discard)) want = sizeof(discard);
			nRet = unzReadCurrentFile(Zip, discard, want);
			if (nRet <= 0) { unzCloseCurrentFile(Zip); bChunkFileOpen = false; return 1; }
			nChunkOffset += (UINT32)nRet;
		}
	}

	INT32 total = 0;
	while (total < nLen) {
		nRet = unzReadCurrentFile(Zip, Dest + total, nLen - total);
		if (nRet < 0) { unzCloseCurrentFile(Zip); bChunkFileOpen = false; return 1; }
		if (nRet == 0) break;
		total += nRet;
	}
	nChunkOffset += (UINT32)total;
	if (pnWrote) *pnWrote = total;

	if (nChunkOffset >= nChunkLength) {
		nRet = unzCloseCurrentFile(Zip);
		bChunkFileOpen = false;
		if (nRet == UNZ_CRCERROR) return 2;
		if (nRet != UNZ_OK) return 1;
	}
	return (total == nLen) ? 0 : 1;
}
