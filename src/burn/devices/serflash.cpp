...
#ifdef __PS3__
static FILE *m_region_file;
static INT32 flash_page_read(UINT32 row, UINT8 *dst, INT32 len)
{
	if (m_region_file) {
		if (fseek(m_region_file, (long)(row * m_flash_page_size), SEEK_SET)) return 1;
		return fread(dst, 1, len, m_region_file) == (size_t)len ? 0 : 1;
	}
	memcpy(dst, m_region + row * m_flash_page_size, len);
	return 0;
}
static void flash_page_write(UINT32 row, const UINT8 *src, INT32 len)
{
	if (m_region_file) {
		if (!fseek(m_region_file, (long)(row * m_flash_page_size), SEEK_SET)) { fwrite(src, 1, len, m_region_file); fflush(m_region_file); }
	} else memcpy(m_region + row * m_flash_page_size, src, len);
}
#endif
...
#ifdef __PS3__
INT32 serflash_init_file(const char *path, INT32 length)
{
	const char mode[] = {'r','+','b',0};
	m_region_file = fopen(path, mode);
#if 0
	m_region_file = fopen(path, \"r+b\");
	if (!m_region_file) return 1;
#endif
	if (!m_region_file) return 1;
	serflash_init(NULL, length);
	return 0;
}
#endif
...
#ifdef __PS3__
	if (m_region_file) fclose(m_region_file);
	m_region_file = NULL;
#endif
}
		...
		switch (m_flash_cmd_prev)
		{
			case 0x00:  // READ
				if (data == 0x30)
				{
					...
					{
						#ifdef __PS3__
						flash_page_read(m_flash_row, m_flash_page_data, m_flash_page_size);
						#else
			...
			case 0x60: // BLOCK ERASE
				if (data==0xd0)
				{
					...
					if (m_flash_row < m_row_num)
					{
						...
						#ifdef __PS3__
						memset(m_flash_page_data, 0xff, m_flash_page_size); flash_page_write(m_flash_row, m_flash_page_data, m_flash_page_size);
						#else
			...
			case 0x80:
				if (data==0x10)
				{
					...
					{
						#ifdef __PS3__
						m_flashwritemap[m_flash_row] = 1; flash_page_write(m_flash_row, m_flash_page_data, m_flash_page_size);
						#else
...
void serflash_n3d_flash_cmd_write(INT32 offset, UINT8 data)
{
	...
	if (data==0x00)
	{
		...
		{
			#ifdef __PS3__
			flash_page_read(m_flash_addr, m_flash_page_data, m_flash_page_size);
			#else
