...
static K053936GPPixelCallback K053936GP_pixel_callback = NULL;
...
void K053936GP_set_pixel_callback(K053936GPPixelCallback callback)
{
	K053936GP_pixel_callback = callback;
}
	...
	if (blend > 0)
	{
		...
		do {
			do {
				...
				pixel = (K053936GP_pixel_callback
					? K053936GP_pixel_callback(srcx, srcy)
					: BURN_ENDIAN_SWAP_INT16(src_base[offs])) | color_base;
				if (!(pixel & cmask))
					continue;
