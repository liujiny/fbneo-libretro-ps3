...
#include "msm6295.h"
#include "upd7759.h"
extern "C" void fbneo_ps3_probe(const char *message);
...
{
	fbneo_ps3_probe("AerofgtInit enter");
	AllMem = NULL;
	MemIndex(0x200000, 0x800000, 0, 0, 0x080000, 0x100000, 0);
	INT32 nLen = MemEnd - (UINT8 *)0;
	fbneo_ps3_probe("Aerofgt memory size %d");
	if ((AllMem = (UINT8 *)BurnMalloc(nLen)) == NULL) return 1;
	fbneo_ps3_probe("Aerofgt memory allocated");
	memset(AllMem, 0, nLen);
	MemIndex(0x200000, 0x800000, 0, 0, 0x080000, 0x100000, 0);

	{
		...
		if (BurnLoadRom(DrvSndROM[1]  + 0x000000, k++, 1)) return 1;
		fbneo_ps3_probe("Aerofgt all ROMs loaded");
		...
		fbneo_ps3_probe("Aerofgt graphics expanded");
	}


	fbneo_ps3_probe("Aerofgt before SekInit");
	SekInit(0, 0x68000);
	SekOpen(0);
	SekMapMemory(Drv68KROM,			0x000000, 0x07ffff, MAP_ROM);
	...
	SekClose();
	fbneo_ps3_probe("Aerofgt 68000 initialized");

	sprite_lut_mask[0] = 0x7fff;

	standard_sound_system(1, 0x080000, 0x100000);
	fbneo_ps3_probe("Aerofgt sound initialized");
	...
	GenericTilemapSetTransparent(1, 0xf);
	GenericTilemapBuildSkipTable(1, 1, 0xf);
	fbneo_ps3_probe("Aerofgt tilemaps initialized");

	DrvDoReset();
	fbneo_ps3_probe("AerofgtInit complete");
...
static void DrvPaletteUpdate() // 5rgb
{
	...
	{
		INT32 color = BURN_ENDIAN_SWAP_INT16(p[i]);
