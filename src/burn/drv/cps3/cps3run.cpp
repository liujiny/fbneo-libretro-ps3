...
#ifdef __PS3__
		if (main_flash.flash_mode == FM_NORMAL)
			retvalue = *(UINT32 *)(RomGame_D + (addr & 0x00ffffff)) ^ cps3_mask((addr & 0x00ffffff) + 0x06000000, cps3_key1, cps3_key2);
#else
		if ( main_flash.flash_mode == FM_NORMAL )
...
#ifdef __PS3__
	bprintf(PRINT_IMPORTANT, _T("[FBNeo] CPS3 compact layout: data=%u, shared-program=%u, saved=%u, main=%d bytes\n"), cps3_data_rom_size, 0x1000000, 0x1000000, nLen);
	RomUser = (UINT8 *)BurnMalloc(cps3_data_rom_size);
	RomGame_D = (UINT8 *)BurnMalloc(0x1000000);
	RomGame = RomGame_D; // decrypt in place on PS3, saving 16 MiB
	Mem = (UINT8 *)BurnMalloc(nLen);
	if (RomUser == NULL || RomGame_D == NULL || Mem == NULL) {
		bprintf(PRINT_ERROR, _T("[FBNeo] CPS3 compact allocation FAILED: user=%p program=%p main=%p\n"), RomUser, RomGame_D, Mem);
		BurnFree(RomUser); BurnFree(RomGame_D); BurnFree(Mem);
		RomGame = NULL;
		return 1;
	}
	memset(RomUser, 0, cps3_data_rom_size);
	memset(RomGame_D, 0, 0x1000000);
	bprintf(PRINT_IMPORTANT, _T("[FBNeo] CPS3 compact allocation OK (shared program ROM)\n"));
#else
	bprintf(PRINT_IMPORTANT, _T("[FBNeo] CPS3 memory layout: data ROM=%u bytes, main block=%d bytes\n"), cps3_data_rom_size, nLen);
	Mem = (UINT8 *)BurnMalloc(nLen);
