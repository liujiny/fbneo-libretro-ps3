...
#if defined(__PS3__) || defined(__CELLOS_LV2__)
#define GAIAPOLS_ROZ_CACHE_SLOTS 16384
static INT32 gaiapols_roz_cache_tag[GAIAPOLS_ROZ_CACHE_SLOTS];
static UINT16 gaiapols_roz_cache_code[GAIAPOLS_ROZ_CACHE_SLOTS];
static UINT16 gaiapols_roz_cache_color[GAIAPOLS_ROZ_CACHE_SLOTS];

static void GaiapolisRozCacheReset()
{
	for (INT32 i = 0; i < GAIAPOLS_ROZ_CACHE_SLOTS; i++) {
		gaiapols_roz_cache_tag[i] = -1;
	}
}

static UINT16 GaiapolisRozFetchPixel(INT32 x, INT32 y)
{
	static INT32 fetch_probe = 0;
	if (fetch_probe == 0 || fetch_probe == 1000 || fetch_probe == 100000)
	fetch_probe++;
	x &= 0x1fff;
	y &= 0x1fff;

	const INT32 tile = (y >> 4) * 512 + (x >> 4);
	const INT32 slot = (tile ^ (tile >> 9)) & (GAIAPOLS_ROZ_CACHE_SLOTS - 1);

	if (gaiapols_roz_cache_tag[slot] != tile)
	{
		UINT8 *dat1 = DrvGfxROM3;
		UINT8 *dat2 = DrvGfxROM3 + 0x20000;
		UINT8 *dat3 = DrvGfxROM3 + 0x60000;
		INT32 code = dat3[tile] | ((dat2[tile] & 0x3f) << 8);
		INT32 color = (tile & 1) ? (dat1[tile >> 1] & 0x0f)
			: (dat1[tile >> 1] >> 4);
		if (dat2[tile] & 0x80) color |= 0x10;
		color <<= 4;

		gaiapols_roz_cache_code[slot] = code;
		gaiapols_roz_cache_color[slot] = color;
		gaiapols_roz_cache_tag[slot] = tile;
	}

	return DrvGfxROM2[(gaiapols_roz_cache_code[slot] << 8)
		| ((y & 0x0f) << 4) | (x & 0x0f)]
		+ gaiapols_roz_cache_color[slot];
}
#endif
...
#if defined(__PS3__) || defined(__CELLOS_LV2__)
// Gaiapolis needs the service switch held across reset once to initialise EEPROM.
static INT32 ps3_gaiapols_eeprom_boot;
#endif
...
static INT32 MemIndex()
{
	...
	DrvGfxROMExp2	= Next;
#if defined(__PS3__) || defined(__CELLOS_LV2__)
	/* Gaiapolis has no K053250 expanded graphics and never references
	 * DrvGfxROMExp2. Omitting this unused 8 MB keeps AllMem inside the
	 * fixed 64 MB PS3 pool instead of requiring a second huge allocation. */
	if (nGame != 5)
#endif
		Next += 0x800000;
...
static INT32 GaiapolisInit()
{
	...
	#if defined(__PS3__) || defined(__CELLOS_LV2__)
		/* The original 8192x8192 pre-render requests 256 MB. Keep only a
		 * harmless source token; K053936 fetches visible pixels through the
		 * cached tile callback instead. */
		if ((pMystwarrRozBitmap = (UINT16*)BurnMalloc(128)) == NULL) return 1;
		memset(pMystwarrRozBitmap, 0, 128);
		K053936_external_bitmap = pMystwarrRozBitmap;
		GaiapolisRozCacheReset();
		K053936GP_set_pixel_callback(GaiapolisRozFetchPixel);
	#else
	...
	K054539_set_gain(0, 2, 0.80);
	K054539_set_gain(0, 3, 0.80);
	K054539_set_gain(0, 4, 1.00);
	K054539_set_gain(0, 5, 1.00);
	K054539_set_gain(0, 6, 1.00);
	K054539_set_gain(0, 7, 1.00);
...
#if defined(__PS3__) || defined(__CELLOS_LV2__)
static UINT8 gamma_lut_ps3[0x100];
#endif
...
{
#if defined(__PS3__) || defined(__CELLOS_LV2__)
	// Automatic equivalent of holding Service (F2) while tapping Reset (F3).
	// Keep Service asserted for three seconds, then reboot normally with the
	// newly-created EEPROM. This runs only for Gaiapolis and once per load.
	if (nGame == 5 && ps3_gaiapols_eeprom_boot < 182) {
		if (ps3_gaiapols_eeprom_boot == 0) {
			DrvService[0] = 1;
			DrvReset = 1;
		} else if (ps3_gaiapols_eeprom_boot < 180) {
			DrvService[0] = 1;
			DrvReset = 0;
		} else if (ps3_gaiapols_eeprom_boot == 180) {
			DrvService[0] = 0;
